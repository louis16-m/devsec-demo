...
def lockout_message(attempt):
    if attempt and attempt.is_locked():
        remaining = (
            int((attempt.locked_until - timezone.now()).total_seconds() // 60) + 1
        )
        return (
...
def register_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            audit_log(
                "auth.registration", request, user, {"assigned_role": ROLE_STANDARD}
            )
            audit_log(
                "auth.role.assigned", request, user, {"assigned_role": ROLE_STANDARD}
            )
            messages.success(
...
class AuditPasswordResetView(auth_views.PasswordResetView):
    def form_valid(self, form):
        ...
        audit_log(
            ...
            self.request,
            user,
            {"email": email, "user_exists": bool(user)},
        )
        return response
...
def serve_uploaded_file(request, user_id, file_type):
    ...
    try:
        response = FileResponse(
            file_field.open("rb"),
            content_type=mimetypes.guess_type(file_field.name)[0]
    ...
    if file_type == "document":
        response["Content-Disposition"] = (
            f'attachment; filename="{os.path.basename(file_field.name)}"'
        )
    else:
