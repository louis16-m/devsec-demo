...
_extract_issue_numbers_from_related_section = (
    MODULE._extract_issue_numbers_from_related_section
)
...
class ValidatePrSubmissionTests(unittest.TestCase):
    ...
    def test_extract_issue_number_allows_plain_hash_reference(self) -> None:
        body = VALID_BODY.replace("Closes #12", "Issue: #12")
        self.assertEqual(_extract_issue_numbers_from_related_section(body), [12])
    ...
    def test_assignment_submission_detected_from_assignment_base_branch(self) -> None:
        self.assertTrue(
            _is_assignment_submission("", "assignment/secure-password-reset")
        )
    ...
    def test_assignment_submission_detected_with_descriptive_branch_text(self) -> None:
        body = VALID_BODY.replace(
            "- assignment/secure-password-reset",
            "- Required branch: assignment/secure-password-reset",
        )
        self.assertTrue(_is_assignment_submission(body, "main"))

    def test_non_assignment_pr_is_not_detected_from_plain_body(self) -> None:
        self.assertFalse(
            _is_assignment_submission(
    ...
    def test_assignment_linking_accepts_matching_issue_and_branch(self) -> None:
        ...
        try:
            self.assertEqual(
                _validate_assignment_linking(
                    VALID_BODY, "assignment/secure-password-reset"
                ),
                [],
            )
        finally:
    ...
    def test_assignment_linking_rejects_wrong_base_branch(self) -> None:
        ...
        MODULE._fetch_issue_body = lambda issue_number: VALID_ISSUE_BODY
        try:
            errors = _validate_assignment_linking(
                VALID_BODY, "assignment/fix-open-redirects"
            )
        finally:
            MODULE._fetch_issue_body = original_fetch
        ...
        self.assertTrue(
            any(
                "requires branch 'assignment/secure-password-reset'" in error
                for error in errors
            )
        )
    ...
    def test_assignment_linking_requires_exactly_one_issue(self) -> None:
        ...
        self.assertTrue(
            any("exactly one assignment issue" in error for error in errors)
        )

    def test_assignment_linking_requires_issue_branch_marker(self) -> None:
        original_fetch = MODULE._fetch_issue_body
        MODULE._fetch_issue_body = (
            lambda issue_number: "## Learning objective\n- Missing branch marker.\n"
        )
        try:
            errors = _validate_assignment_linking(
                VALID_BODY, "assignment/secure-password-reset"
        ...
        self.assertTrue(
            any(
                "missing a valid 'Required submission branch' section" in error
                for error in errors
            )
        )
...
"""
    ...
    def test_protected_path_validation_skips_non_assignment_prs(self) -> None:
        ...
        try:
            self.assertEqual(
                _validate_protected_assignment_paths(
                    "## Summary\n- Internal instructor maintenance update.\n", "main"
                ),
                [],
            )
    ...
    def test_protected_path_validation_rejects_assignment_prs_touching_admin_files(
        self,
    ) -> None:
        original_fetch = MODULE._fetch_pull_request_files
        ...
        try:
            errors = _validate_protected_assignment_paths(
                VALID_BODY, "assignment/secure-password-reset"
            )
        finally:
            MODULE._fetch_pull_request_files = original_fetch
        ...
        self.assertTrue(
            any("must not modify instructor-facing" in error for error in errors)
        )
        self.assertTrue(
            any(
                "Restricted file changed: .github/workflows/ci.yml" == error
                for error in errors
            )
        )
        self.assertTrue(
            any(
                "Restricted file changed: scripts/create_assignment_issues.sh" == error
                for error in errors
            )
        )
